...
#		sys.stderr.write("Logfile can be found here: %s\n\n" % log)
	...
	def getJobsOutput(self, ids):
		...
		try:
			...
			if len(ids) == 1:
				...
				tmpPath = os.path.join(basePath, wmsExtraDir)
		...
		if retCode != 0:
			...
			self.printError(self._outputExec, retCode, stderr, log, [jobs, tmpPath])
			if "Output files already retrieved" in str.join('\n', stderr):
	...
	def cancelJobs(self, ids):
		...
		try:
			...
			deleted = map(str.strip('- \n'), filter(lambda x: x.startswith('- '), lines))
			deleted.sort()

			if (retCode != 0) or (deleted != ids):
