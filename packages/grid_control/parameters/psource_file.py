...
from python_compat import identity, ifilter, imap, izip, json, lfilter, lmap, sorted
...
class CSVParameterSource(InternalParameterSource):  # Reader for CSV files
	...
	def __init__(self, fn, format='sniffed'):
		...
		keys = sorted(imap(ParameterMetadata, lfilter(identity, csv_header)), key=lambda k: k.value)
		InternalParameterSource.__init__(self, lmap(_cleanup_dict, psp_list), keys)
