...
class CreamWMS(GridWMS):
	...
	def __init__(self, config, name):
		cancelExecutor = CancelAndPurgeJobs(config,
			CancelJobsWithProcessBlind(config, 'glite-ce-job-cancel', ['--noint', '--logfile', '/dev/stderr']),
			CancelJobsWithProcessBlind(config, 'glite-ce-job-purge', ['--noint', '--logfile', '/dev/stderr']))
