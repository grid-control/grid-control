...
class CancelJobs(BackendExecutor):
	def execute(self, wmsIDs, wmsName): # yields list of (wmsID, job_status)
		raise AbstractError
...
class CancelAndPurgeJobs(CancelJobs):
	...
	def execute(self, wmsIDs, wmsName): # yields list of (wmsID, job_status, job_info)
		marked_wmsIDs = list(self._cancel_executor.execute(wmsIDs, wmsName))
		time.sleep(5)
		activity = utils.ActivityLog('purging jobs')
		for wmsID in self._purge_executor.execute(marked_wmsIDs, wmsName):
			yield wmsID
		activity.finish()
