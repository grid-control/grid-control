	...
	def resync_partitions(self, path, block_list_old, block_list_new):
		...
		try:
			resync_partition_iter = self._partition_resync_handler.resync(
				block_list_old, block_list_new, pnum_list_redo, pnum_list_disable, self)
			path_tmp = path + '.tmp'
...
class PartitionResyncHandler(ConfigurablePlugin):
	...
	def resync(self, block_list_old, block_list_new, pnum_list_redo, pnum_list_disable, splitter):
		block_resync_state = BlockResyncState(block_list_old, block_list_new)
		# User overview and setup starts here
		resync_info_iter = self._iter_resync_infos(splitter, block_resync_state)
	...
	def _expand_outside(self, splitter, fi_idx, partition_num, size_list,
		...
		iter_partitions_added = splitter.partition_blocks_raw([block_new],
	...
	def _handle_changed_file(self, splitter, proc_mode, fi_idx, partition_mod, partition_num, size_list,
		...
		if self._handle_changed_entries(splitter, fi_idx, partition_mod, partition_num,
				size_list, fi_old, fi_new, block_new, partition_list_added):
	...
	def _iter_resync_infos(self, splitter, block_resync_state):
		...
		if self._mode_added == ResyncMode.complete:
			for partition_added in splitter.partition_blocks_raw(block_resync_state.block_list_added):
				yield (None, partition_added, ResyncMode.ignore)
