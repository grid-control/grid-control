	...
	def getOptions(self, container, section):
		if isinstance(section, list):
			...
			for s in section: # Collect options from multiple sections
				result += container.getOptions(s)
			return utils.uniqueListRL(sorted(result))
		return sorted(container.getOptions(section))
...
class Config(ConfigBase):
	def __init__(self, configFile = None, configDict = {}, optParser = None, configHostSpecific = True):
		...
		# Setup config interface for following getPath,... calls
		def mySet(section, option, value, *args, **kwargs):
			return self.setChecked(lambda cc: self._resolver.getTarget(cc, section, option), value, *args, **kwargs)
		def myGet(desc, obj2str, str2obj, section, option, *args, **kwargs):
			return self.getTyped_compat(desc, obj2str, str2obj,
				lambda cc: self._resolver.getSource(cc, section, option), *args, **kwargs)
		ConfigBase.__init__(self, mySet, myGet, self.getOptions, self._baseDir)
		...
		self.workDir = self.getPath('global', 'workdir', os.path.join(wdBase, 'work.' + confName),
			mustExist = False, markDefault = False)

		# Determine and load stored config settings
		self._flatCfgPath = os.path.join(self.workDir, 'current.conf') # Flattened config file
		...
		# Get persistent variables (config ID, work directory, ...) - only possible after self._oldCfg was set!
	...
	def freezeConfig(self, writeConfig = True):
		...
		for entry in unused:
			self._logger.log(logging.INFO1, '\t%s' % entry.format(section = True))
		if writeConfig:
	...
	def getScoped(self, section):
		return ConfigBase(self.set, self.getTyped, self.getOptions)
