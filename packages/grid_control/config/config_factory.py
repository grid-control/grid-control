...
class ConfigFactory(object):
	...
	def __init__(self, filler = None, config_file_path = None):
		...
		path_work = tmp_config.getPath('workdir', os.path.join(workdir_base, get_name('work')), must_exist = False)
	...
	def freeze(self, write_config = True):
		...
		unused = lfilter(lambda entry: ('!' not in entry.section) and not entry.accessed, self._view.iter_content())
		...
		if write_config or not os.path.exists(self._config_path_old):
			...
			self._write_file(self._config_path_flat, print_default = False, print_unused = False, print_minimal = True,
				print_workdir = True)
			self._write_file(self._config_path_old,  print_default = True,  print_unused = True,  print_minimal = True, print_source = True,
