...
from config_entry import ConfigContainer
from config_interface import TypedConfigInterface
from cview_base import SimpleConfigView
from cfiller_base import FileConfigFiller, CompatConfigFiller
...
class ConfigFactory(object):
	def __init__(self, filler = None, configFilePath = None):
		...
		self._view.pathDict['search'] = [os.getcwd(), pathMain]
		...
		if os.path.exists(self._oldCfgPath):
			FileConfigFiller([self._oldCfgPath]).fill(oldContainer)
	...
	def freezeConfig(self, writeConfig = True):
		...
		if writeConfig or not os.path.exists(self._oldCfgPath):
			...
			self._view.write(open(self._flatCfgPath, 'w'), printDefault = False, printUnused = False, printMinimal = True)
