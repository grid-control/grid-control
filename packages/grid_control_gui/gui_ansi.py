...
from grid_control.utils import abort, guard
...
from hpfwk import DebugInterface, ExceptionCollector, NestedException
...
class FrameGUIElement(GUIElement):
	...
	def draw_init(self):
		...
		stdstream_buffer = StringBuffer()  # hide all logging output
		GCStreamHandler.push_std_stream(stdstream_buffer, stdstream_buffer)
	...
	def draw_finish(self):
		try:
			...
			if self._dump_stream:  # display logging output
				sys.stderr.write(stdstream_buffer.getvalue())
	...
	def set_layout(self, pos, height, width):
		return self._element.set_layout(pos, height, width)
...
class ANSIGUI(GUI):
	...
	def start_display(self):
		DebugInterface.callback_list.append((self._destroy, self._create))
		try:
			guard(self._create, self._destroy, self._workflow.process)
		finally:
			DebugInterface.callback_list.remove((self._destroy, self._create))
		self._exc.raise_any(NestedException('GUI drawing exception'))
