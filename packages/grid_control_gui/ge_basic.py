...
class ActivityGUIElement(AfterImageGUIElement):
	...
	def __init__(self, config, name, workflow, redraw_event, stream):
		...
		self._activity_monitor = config.get_plugin('activity stream', 'multi_stream', on_change=None,
			cls=ActivityMonitor, pargs=(self._buffer,))
	...
	def _update_buffer(self):
		...
		erase_content(self._buffer)
		self._height_last = 1 + message.count('\n')
