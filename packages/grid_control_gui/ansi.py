...
def create_ansi_fun(fmt_str, fmt_args_fun, fmt_str_low_color=None):
	term_dumb = is_dumb_terminal()  # False = high_color, None = low_color, True = dumb
	if ((term_dumb is False) or (fmt_str_low_color is None)) and not term_dumb:
		def _fmt_fun(*args):
			return fmt_str % fmt_args_fun(*args)
	else:
		fmt_str = create_ansi(fmt_str_low_color)
...
class ANSI(object):
	...
	color_grayscale = create_ansi_fun(chr(27) + '[38;5;%dm',
		lambda value: 232 + int(value * 23), fmt_str_low_color=chr(27) + '[37m')
...
class Console(object):
	...
	def getmaxyx(cls):
		def _getmaxyx(fd_term=sys.stdout.fileno()):
			winsize_ptr = fcntl.ioctl(fd_term, termios.TIOCGWINSZ, struct.pack("HHHH", 0, 0, 0, 0))
