...
def _geo_resolve():
	...
	from python_compat import set, imap, lfilter, sorted, lmap

	def _geocode(jrc, loc):
		result = jrc.get(params={'address': str.join('.', loc.split('.')[2:]), 'sensor': 'false'})
		if 'Placemark' in result:  # unfold placemake entries
			result = lmap(lambda x: (x['address'], tuple(reversed(x['Point']['coordinates'][:2]))),
				result['Placemark'])
		return result
