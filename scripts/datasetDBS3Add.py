...
def generateDBS3BlockDumps(provider):
	for blockInfo in provider.getBlocks():
		...
		for fileInfo in fileList:
			...
			makeLumiDict = lambda (run_num, lumi_num): {'lumi_section_num': lumi_num, 'run_num': run_num}
			fileLumiList = map(makeLumiDict, metadataInfo['CMSSW_LUMIS'])

			###add file information
			fileDict = {'check_sum': metadataInfo['SE_OUTPUT_HASH_CRC32'],
						'file_lumi_list': fileLumiList,
						'adler32': 'NOTSET',
						'event_count': metadataInfo['CMSSW_EVENTS_WRITE'],
						'file_type': 'EDM',
						'logical_file_name': lfn,
						'file_size': fileSize,
						'md5': metadataInfo['SE_OUTPUT_HASH_MD5'],
						'auto_cross_section': 0.0
						}
			blockDump['files'].append(fileDict)

			###add file configurations
			fileConfDict = {'release_version': metadataInfo['CMSSW_VERSION'],
							'pset_hash': metadataInfo['CMSSW_CONFIG_HASH'],
							'lfn': lfn,
							'app_name': 'cmsRun',
							'output_module_label': 'crab2_mod_label',
							'global_tag': metadataInfo['GLOBALTAG'],
							}

			blockDump['file_conf_list'].append(fileConfDict)
			...
			blockDump['file_parent_list'].extend(file_parentage)

			###fill dataset configurations
			datasetConfDict = dict(fileConfDict)
			del datasetConfDict['lfn']
		...
		blockDump['primds'] = {'primary_ds_type': datasetType, 'primary_ds_name': primaryDataset}

		###add dataset information
		blockDump['dataset'] = {'physics_group_name': None,
								 'dataset_access_type': 'VALID',
								 'data_tier_name': dataTier,
								 'processed_ds_name': processedDataset,
								 'xtcrosssection': None,###Add to meta data from FrameWorkJobReport, if possible!
								 'dataset': dataset
								 }

		###add block information
		blockDump['block'] = {'open_for_writing': 0,
							   'block_name': '%s#%s' % (dataset, blockName),
							   'file_count': len(fileList),
							   'block_size': blockSize,
							   'origin_site_name': locations[0] if len(locations) else 'UNKNOWN'}

		###add acquisition_era, CRAB is important because of checks within DBS 3
		blockDump['acquisition_era'] = {'acquisition_era_name': 'CRAB', 'start_date': 0}
		###add processing_era
		blockDump['processing_era'] = {'processing_version': 1, 'description': 'grid-control'}
		###add dataset configurations
		blockDump['dataset_conf_list'] = list(datasetConfigurations)
