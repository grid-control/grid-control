...
from testFwk import TestStream, create_workflow, display_jobs, remove_files_testsuite, run_test, str_dict_testsuite, try_catch
...
def checkTask(workflow):
	workflow.jobManager.check(workflow.task, None)
	task = workflow.task
...
def local_create_workflow(settings):
	return create_workflow(userConfigDict = settings, abort = 'jobmanager')

def display_report(workflow):
	stream = TestStream()
	def write_line(content, width = 65, newline = True):
		content = content.ljust(width)
		if newline:
			content += '\n'
		if 'jobs' in content.lower():
			stream.write(content)
	report = Report.create_instance('BasicReport', workflow.jobManager.jobDB, workflow.task)
	report._write_line = write_line
	report.show_report(workflow.jobManager.jobDB)

def do_resync_test(settings, resync_time = 0, report = True):
	workflow = local_create_workflow(settings)
	print('____')

	def display():
		checkTask(workflow)
		print('____')
		display_jobs(workflow.jobManager.jobDB)
		if report:
			print('____')
...
class Test_TaskResync:
	"""
	>>> remove_files_testsuite(files_list)
	...
	Jobs assigned to WMS:        0        Failing jobs:       0    0%
	...
	Dataset source will be queried every 0h 00min 01sec
	Current task ID: GC000000000000
	Task started on: 0000-00-00
	Using batch system: HOST
	____
...
class Test_TaskResyncDataset:
	...
	>>> try_catch(lambda: local_create_workflow({'task': {'dataset': 'data_resync0.dbs', 'files per job': 1}}), 'UserError', 'Currently used dataset does not provide jobs to process')
	caught

	>>> workflow_store = do_resync_test({'task': {'dataset': 'data_resync0.dbs', 'files per job': 1, 'dataset refresh': '1:00'}})
	Dataset source will be queried every 1h 00min 00sec
	...
	Dataset source will be queried every 0h 01min 00sec
	Currently used dataset does not provide jobs to process
...
class Test_TaskResyncMultiDataset:
	...
	>>> dummy = do_resync_test({'task': {'files per job': 1, 'datasource names': 'data1 data2', 'data1': 'data_resync1.dbs', 'data2': 'data_resync2.dbs', 'data1 refresh': '0:00:01', 'dataset default query interval': '00:00:01'}}, 1, report = False)
	Dataset source will be queried every 0h 00min 01sec
	Current task ID: GC000000000000
	Task started on: 0000-00-00
	Using batch system: HOST
...
run_test(exit_fun = lambda: remove_files_testsuite(files_list))
