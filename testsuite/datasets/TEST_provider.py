...
# - prolog marker
import logging
from testFwk import cmp_obj, create_config, remove_files_testsuite, run_test, try_catch
...
}).change_view(setSections = ['datasetexample'])
...
def create_multi(value, defaultProvider):
	config = create_config(config_dict = {'section': {'dataset provider': defaultProvider}})
	dp_list = []
...
class Test_DataProviderBind:
	...
	<instance factory for grid_control.datasets.provider_basic.ListProvider(<SimpleConfigInterface(view = <SimpleConfigView(sections = None)>)>, 'dataset', 'file.dbs', '', NullDataProcessor(), ...)>
	<instance factory for grid_control.datasets.provider_basic.ListProvider(<SimpleConfigInterface(view = <SimpleConfigView(sections = None)>)>, 'dataset', 'file.dbs', 'nick', NullDataProcessor(), ...)>
	<instance factory for grid_control.datasets.provider_basic.FileProvider(<SimpleConfigInterface(view = <SimpleConfigView(sections = None)>)>, 'dataset', 'file.dbs', 'nick', NullDataProcessor(), ...)>

	>>> test_bind(DP.bind('file.dbs \\n nick : file.dbs \\n nick: file : file.dbs', config = create_config(config_dict = {'dataset': {'dataset provider': 'FileProvider'}})))
	<instance factory for grid_control.datasets.provider_basic.FileProvider(<SimpleConfigInterface(view = <SimpleConfigView(sections = None)>)>, 'dataset', 'file.dbs', '', NullDataProcessor(), ...)>
	<instance factory for grid_control.datasets.provider_basic.FileProvider(<SimpleConfigInterface(view = <SimpleConfigView(sections = None)>)>, 'dataset', 'file.dbs', 'nick', NullDataProcessor(), ...)>
	<instance factory for grid_control.datasets.provider_basic.FileProvider(<SimpleConfigInterface(view = <SimpleConfigView(sections = None)>)>, 'dataset', 'file.dbs', 'nick', NullDataProcessor(), ...)>
...
class Test_ListProvider:
	...
	>>> remove_files_testsuite(['dataA1.dbs'])
	...
	True
	>>> remove_files_testsuite(['dataRL1.dbs'])
	"""
