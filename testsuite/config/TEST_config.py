...
class Test_Config:
	...
	>>> config.write(TestStream(), print_default = False, print_unused = False)
	[global]
	...
	>>> config.write(TestStream(), print_default = True, print_unused = False)
	[dict]
	test = works
	-----
	[global]
	...
	>>> config.get('key')
	'value'
	>>> config.change_view(setSections = ['TEST', 'TESTnick']).get('key')
	'valueX'
	>>> config.change_view(setSections = ['TEST', 'TESTnicky']).get('key')
	'value'
	...
	>>> config.change_view(setSections = ['TEST']).get_list('liste')
	['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h']

	>>> config.change_view(setSections = ['TEST', 'TESTnick']).get_list('liste')
	['x', 'y', 'z']

	>>> l0 = [1, 2]
	>>> config1 = config.change_view(setSections = ['TEST'])
...
class Test_Interpolation:
	...
	>>> config.get('key_t4')
	'test2_2 test2_1'
	>>> config.write(TestStream(), print_unused = False)
	[global!]
	config id ?= unnamed
	plugin paths ?= <testsuite dir>/config
...
class Test_ConfigScoped:
	...
	>>> config_nt = config.change_view(setSections = ['section named', 'section test'])
	...
	>>> config_s = config.change_view(setSections = ['section'])
	...
	>>> config_n = config.change_view(setSections = ['section named'])
	...
	>>> config_t = config.change_view(setSections = ['section test'])
	...
	>>> config_st = config.change_view(setSections = ['section', 'section test'])
	...
	>>> config_sn = config.change_view(setSections = ['section', 'section named'])
	...
	>>> config_snt = config.change_view(setSections = ['section', 'section named', 'section test'])
	...
	>>> config = create_config('sections.conf')
	>>> config.change_view(setSections = ['section']).get('keyX', 'persistent1')
	'persistent1'
	>>> config.change_view(setSections = ['section named']).get('keyX', 'persistent2')
	'persistent2'
	>>> config.change_view(setSections = ['section named', 'section']).get('keyX')
	'persistent1'
	>>> config.change_view(setSections = ['section', 'section named']).get('keyX')
	'persistent2'
