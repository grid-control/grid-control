...
# - prolog marker
from testFwk import TestStream, create_config, remove_files_testsuite, run_test, write_file
...
class WMS(NamedPlugin):
	...
	def __init__(self, config, name):
		config = config.change_view(setSections = ['global', 'dataset'])
...
class Test_ConfigWMS(object):
	...
	>>> config = config.change_view(view_class = 'TaggedConfigView', setSections = ['global'], setTags = [Task(config, 'datasettask')])
	>>> config.get_plugin('wms', cls = WMS)
	PBS(mybatch mypbs)
	>>> config.get_plugin('wms', cls = WMS, tags = [Task(config, 'mytag')])
	PBS(mybatch mypbs tasktag:mytag)
	>>> config = config.change_view(setSections = None)

	>>> config.write(TestStream(), print_source = True)
	[backend]
...
run_test(cleanup_fun = lambda: remove_files_testsuite(['configfile.conf']))
