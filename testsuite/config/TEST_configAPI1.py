...
class Test_ConfigViews:
	...
	>>> view1 = config.change_view(view_class = 'TaggedConfigView', setNames = ['name1'], setClasses = [Test1], setTags = [test1])
	>>> view1
	<SimpleConfigInterface(view = <TaggedConfigView(class = ['section1', 'section2'], sections = None, names = ['name1'], tags = [('tag', 'test1')])>)>
	>>> view2 = config.change_view(view_class = 'TaggedConfigView', setNames = ['name2'], setClasses = [Test2], setTags = [test1, test2])
	>>> view2
	<SimpleConfigInterface(view = <TaggedConfigView(class = ['section3', 'section4'], sections = None, names = ['name2'], tags = [('tag', 'test1'), ('tag', 'test2')])>)>
	>>> try_catch(lambda: config.change_view(view_class = 'TaggedConfigView', setTags = [view1]), 'APIError', 'does not define a valid tag name')
	caught
	...
	>>> config.write(TestStream(), print_source = False, print_minimal = True, print_default = False)
	[global]
	plugin paths += <testsuite dir>/config
	-----
	[section1]
...
run_test(cleanup_fun = lambda: remove_files_testsuite(['configfile0.conf', 'configfile1.conf', 'configfile2.conf', 'configfileA.conf']))
