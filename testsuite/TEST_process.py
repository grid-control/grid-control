...
class Test_LocalProcess:
	...
	caught
	>>> p6.status(timeout = 1)
	>>> p6.restart(timeout = 10)
	'SIGKILL'
	>>> p6.get_runtime() < 10
	True
	>>> p6.status(timeout = 0)

	>>> 'Hello World' in p6.stdout.read_cond(timeout = 5, cond = lambda buffer: 'Hello World' in buffer)
	True
	>>> p6
	...
	caught
	>>> p6.status(timeout = 5, terminate = True) # delay kill so 'TEST...' is in the output buffer
	'SIGKILL'
	>>> p6.get_runtime() > 1
	True
