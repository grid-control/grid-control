...
# - prolog marker
import os, re, sys, signal, logging
from testFwk import DummyObj, TestStream, create_config, get_logger, run_test, try_catch
from grid_control.gc_exceptions import GCLogHandler, gc_excepthook
from grid_control.logging_setup import GCFormatter, StdoutStreamHandler
from hpfwk import NestedException
from hpfwk.hpf_debug import create_debug_console, handle_dump_interrupt
from hpfwk.hpf_exceptions import impl_detail


gc_excepthook.restore = False
...
class Test_Exception:
	...
	0000-00-00 00:00:00 - exception:CRITICAL - Exception occured in grid-control...
	-----
	NestedException: Tier3 - ZeroDivisionError: <details>

	>>> old_handler = signal.signal(signal.SIGUSR1, handle_dump_interrupt)
	>>> os.kill(os.getpid(), signal.SIGUSR1)
	# active threads 1
	Stack of threads...

	>>> con = create_debug_console({'key': 'value'})
	>>> con.locals['key'] == 'value'
	True
