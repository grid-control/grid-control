...
from python_compat import identity, ifilter, imap, irange, lfilter, lmap, md5_hex, set, sorted
...
def testPA(pa, showJob = True, showPNum = True, showMetadata = True, showJobPrefix = True,
	...
	if showKeys:
		print('Keys = %s' % str.join(', ', imap(lambda key: '%s%s' % (key.value, utils.QM(key.untracked, '', ' [trk]')), keys)))
...
class DataSplitProcessorTest(PartitionProcessor):
	...
	def process(self, pNum, partition, result):
		...
		result[ParameterInfo.ACTIVE] = result[ParameterInfo.ACTIVE] and not partition.get(DataSplitter.Invalid, False)
