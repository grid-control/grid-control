...
from testINC import DataSplitProcessorTest, TestParameterSource, norm_ps_display, testPS, updateDS
...
class Test_DataParameterSourceReal:
	"""
	>>> utils.remove_files(['datasetcache.dat', 'datasetmap.tar'])
	>>> dataSource = DataProvider.create_instance('ListProvider', create_config(), 'dataset', '../datasets/dataE.dbs', None)
	>>> config = create_config(config_dict = {'dataset': {'events per job': 10}})
	>>> dataSplit = DataSplitter.create_instance('EventBoundarySplitter', config, 'dataset')
	...
	>>> dataProc = PartitionProcessor.create_instance('MultiPartitionProcessor', config, [pproc_basic, pproc_se], 'dataset')
	>>> ps = ParameterSource.create_instance('DataParameterSource', '.', 'dataset', dataSource, dataSplit, dataProc, {}, False)
	Block /MY/DATASET#easy3 is not available at any site!
	>>> testPS(ps)
	data(dataset)
	3f7e48d7652e0815ce2c9b21abcff537
	9
	...
	>>> utils.remove_files(['datasetcache.dat', 'datasetmap.tar'])
	>>> config2 = create_config(config_dict = {'dataset': {'files per job': 10}})
	>>> dataSplit = DataSplitter.create_instance('FileBoundarySplitter', config2, 'dataset')
	>>> ps2 = ParameterSource.create_instance('DataParameterSource', '.', 'dataset', dataSource, dataSplit, pproc_basic, {}, False)
	>>> utils.remove_files(['datasetcache.dat', 'datasetmap.tar'])
	"""
...
class Test_DataParameterSource:
	...
	>>> config = create_config(config_dict = {'datasetx': {'events per job': 3}})
	>>> data_bl = ss2bl('AABBCCCD')
	>>> updateDS(data_bl, '')
	>>> dataSource = DataProvider.create_instance('ListProvider', config, 'dataset', 'dataset.tmp', 'nick')
	>>> dataSplit = DataSplitter.create_instance('EventBoundarySplitter', config, 'dataset')
	>>> dataProc = DataSplitProcessorTest(config, 'dataset')
	>>> repository = {}
	>>> pd = ParameterSource.create_instance('DataParameterSource', '.', 'dummy', dataSource, dataSplit, dataProc, repository, False)
	...
	>>> testPS(pd, showHash = False) # AAB BCC CD
	data(dummy)
	3
	...
	>>> pd.resync_psrc() == (set([]), set([]), True)
	True
	>>> testPS(pd, showHash = False) # AAB BCC CD D
	data(dummy)
	4
	Keys = EVT, FN, SID [trk], SKIP, GC_PARAM
	0 {'<ACTIVE>': True, '<REQS>': [], '!EVT': 3, '!FN': 'A B', 'SID': 0, '!SKIP': 0, '!GC_PARAM': 0}
	...
	>>> pd.resync_psrc() == (set([]), set([1, 2]), True)
	True
	>>> testPS(pd, showHash = False) # AAB BCc cD D BC D
	data(dummy)
	6
	Keys = EVT, FN, SID [trk], SKIP, GC_PARAM
	0 {'<ACTIVE>': True, '<REQS>': [], '!EVT': 3, '!FN': 'A B', 'SID': 0, '!SKIP': 0, '!GC_PARAM': 0}
	...
	>>> pd.resync_psrc() == (set([]), set([3]), False)
	True
	>>> testPS(pd, showHash = False) # AAB BCc cD d BC D
	data(dummy)
	6
	...
	>>> pd.resync_psrc() == (set([]), set([]), True)
	True
	>>> testPS(pd, showHash = False) # AAB BCc cD d BC D CC
	data(dummy)
	7
	Keys = EVT, FN, SID [trk], SKIP, GC_PARAM
	0 {'<ACTIVE>': True, '<REQS>': [], '!EVT': 3, '!FN': 'A B', 'SID': 0, '!SKIP': 0, '!GC_PARAM': 0}
