...
from testINC import display_ps_str2ps_str, norm_ps_display, ps_str2display_ps_str, testPA
...
def DS_setup():
	data_src = createfun_get_list_provider()('test.dbs')
	ps = get_data_psrc(data_src, splitter_name='FileBoundarySplitter', repository=global_repo,
		config_dict={'files per job': 1},
		pproc_name_list=['TestsuitePartitionProcessor'])
...
def getBPF(config_dict = None):
	...
	config = create_config(config_dict = config_dict or {}).change_view(view_class = 'TaggedConfigView', set_classes = [X()])
...
def testSubSpace(pexpr, factory, **kwargs):
	...
	config = create_config(config_dict=config_dict).change_view(
		view_class = 'SimpleConfigView', set_sections = ['parameters'])
...
class Test_Scope:
	"""
	>>> 
	>>> testScope({'parameters': {'parameters': '1'}})
	NullParameterSource:
	>>> testScope({'parameters': {'parameters': '2'}})
	TruncateParameterSource: truncate = 2
	  NullParameterSource:
	>>> testScope({'parameters': {'parameters': '2 * 3'}})
	TruncateParameterSource: truncate = 6
	  NullParameterSource:
	>>> try_catch(lambda: testScope({'parameters': {'parameters': '{}'}}), 'ParameterError', 'Invalid subspace reference')
	caught
	>>> try_catch(lambda: testScope({'parameters': {'parameters': '%'}}), 'ParameterError', 'Invalid stack state detected')
	caught
	>>> try_catch(lambda: testScope({'parameters': {'parameters': '< x >', 'x type': 'fail'}}), 'ParameterError', 'Unknown reference type')
	caught
	>>> try_catch(lambda: testScope({'parameters': {'parameters': '<x'}}), 'ParameterError', 'Parenthesis error')
	caught
	>>> try_catch(lambda: testScope({'parameters': {'parameters': '<x y>'}}), 'ParameterError', 'Invalid arguments for reference')
	caught
	...
	SimpleParameterSource: var = A, len = 2
	>>> testScope({'parameters': {'parameters': 'A B', 'A': '1 2', 'B': 'X\\n 1 => Y', 'default lookup': 'A'}})
