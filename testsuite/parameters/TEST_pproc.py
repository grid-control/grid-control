...
def get_config(settings = None):
	...
	return create_config(config_dict = {'dataset': settings})

def testPP(dataProcNames, config = None, keys = [], dataset = 'dataE.dbs', w = 23):
	utils.remove_files(['datasetcache.dat', 'datasetmap.tar'])
	if not config:
		config = get_config()
	dataProcList = lmap(lambda name: PartitionProcessor.create_instance(name, config, 'dataset'), dataProcNames.split())
	assert(dataProcList)
	if len(dataProcList) > 1:
		dataProc = PartitionProcessor.create_instance('MultiPartitionProcessor', config, dataProcList, 'dataset')
	else:
		dataProc = dataProcList[0]
	dataSource = DataProvider.create_instance('ListProvider', create_config(), 'dataset', '../datasets/' + dataset, None)
	dataSplit = DataSplitter.create_instance('BlockBoundarySplitter', config, 'dataset')
...
def testPPLoc(config = None):
	config = config or {}
	config.setdefault('partition location filter matcher mode', 'end')
	return testPP('LocationPartitionProcessor', get_config(config), dataset = 'dataH.dbs', w = 55)
...
class Test_LFNPartitionProcessor:
	...
	>>> testPP('LFNPartitionProcessor BasicPartitionProcessor', get_config({'partition lfn modifier': 'srm://test.de/store'}), keys = ['DATASET_SRM_FILES'])
	Block /MY/DATASET#easy3 is not available at any site!
...
class Test_TFCPartitionProcessor:
	...
	>>> testPP('TFCPartitionProcessor BasicPartitionProcessor LocationPartitionProcessor',
	... config = get_config({'partition tfc': 'SE4 => xrootd:'}))
	Block /MY/DATASET#easy3 is not available at any site!
	0  True [('STORAGE', ['SE4'])]  xrootd:/path/file0 xrootd:/path/file1 xrootd:/path/file2
	1  True []                      /path/file3 /path/file5
	2 False [('STORAGE', [])]       /path/file6 /path/file7 /path/file8 /path/file9
...
class Test_ReqPartitionProcessor:
	"""
	>>> testPP('BasicPartitionProcessor RequirementsPartitionProcessor', get_config({'partition walltime factor': 2, 'partition walltime offset': 10}))
	Block /MY/DATASET#easy3 is not available at any site!
	...
	>>> testPP('BasicPartitionProcessor RequirementsPartitionProcessor', get_config({'partition cputime factor': 0.2, 'partition cputime offset': 10}))
	Block /MY/DATASET#easy3 is not available at any site!
	...
	>>> testPP('BasicPartitionProcessor RequirementsPartitionProcessor', get_config({'partition memory factor': 100, 'partition memory offset': -2000}))
	Block /MY/DATASET#easy3 is not available at any site!
