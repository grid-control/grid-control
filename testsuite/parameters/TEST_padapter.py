...
from testINC import DataSplitProcessorTest, TestParameterSource, testPA, testPS, updateDS
...
class Test_ParameterAdapter:
	"""
	>>> remove_files_testsuite(['task.dat', 'params.dat.gz', 'params.map.gz'])
...
class Test_TrackedParameterAdapterRepeat:
	"""
	>>> remove_files_testsuite(['task.dat', 'params.dat.gz', 'params.map.gz', 'dummycache*.dat', 'dummymap*.tar', 'dataset.tmp'])
	>>> config = create_config(config_dict={'global': {'workdir': '.'}, 'dataset': {'events per job': 3}})
	...
	>>> pd = ParameterSource.create_instance('DataParameterSource', '.', 'dummy', dataSource, dataSplit, dataProc, {}, False)
